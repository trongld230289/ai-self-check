...
• Use @review-changes to review only git changes instead of entire file`
    ...
    const reviewChangesParticipant = vscode.chat.createChatParticipant('review-changes', async (request, context, stream, token) => {
        try {
            ...
            // Debug in markdown to see what's happening
            stream.markdown('### 🎯 DEBUG: Chat Participant Called\n');
            stream.markdown(`**Request.model found:** ${request.model ? `${request.model.family} (${request.model.vendor})` : 'null'}\n\n`);
            stream.markdown(`**User selected in VS Code Chat:** ${request.model ? request.model.family : 'unknown'}\n\n`);
...
async function handleActiveFileReview(stream, requestedModel = null, context = null) {
    try {
        ...
        // Ensure we always provide meaningful output
        stream.markdown('# 📄 Active File Review\n\n');
        stream.markdown(`**File:** \`${path.basename(document.fileName)}\`\n`);
        stream.markdown(`**Full path:** \`${document.fileName}\`\n`);
        stream.markdown(`**Language:** ${document.languageId}\n`);
        ...
        if (isSelection) {
            ...
            stream.markdown(`**Total Lines:** ${document.lineCount}\n\n`);
            const content = document.getText();
            
            if (!content.trim()) {
                stream.markdown('⚠️ **File is empty**\n\n');
                stream.markdown('💡 **Tip:** Add some code to review\n');
                return;
            }
            
            if (content.length > 3000) {
                stream.markdown('## 📋 File Preview (First 3000 characters)\n\n');
                stream.markdown('```' + document.languageId + '\n' + content.substring(0, 3000) + '\n...\n```\n\n');
                stream.markdown('💡 **Large file detected** - Consider selecting specific sections for detailed review\n\n');
            } else {
                stream.markdown('## 📋 File Content\n\n');
                stream.markdown('```' + document.languageId + '\n' + content + '\n```\n\n');
            }
            
            // Analyze the full file with the selected model
            await analyzeCodeSection(content, document.languageId, stream, requestedModel, context);
        }
...
async function handleInstructionReview(stream, selectedModel = null, context = null, request = null) {
    try {
        ...
        // Dynamic In-Memory Review (new approach)
        stream.markdown('# 🔍 Dynamic Code Review (In-Memory)\n\n');
        stream.markdown('**Method:** Template + Diff + In-Memory Processing\n\n');
        ...
        try {
            ...
            if (!gitDiff.trim()) {
                ...
                if (!stagedDiff.trim()) {
                    ...
                    stream.markdown('📋 **Found staged changes - processing in-memory...**\n\n');
                    await reviewWithInMemoryTemplate(stagedDiff, stream, 'Staged Changes', selectedModel, context, request);
                }
            } else {
                stream.markdown('📋 **Found unstaged changes - processing in-memory...**\n\n');
...
async function getFallbackModel(currentModel, stream, attemptNumber = 1) {
    try {
        ...
        // 1. Claude 4 (Claude 3.5 Sonnet)
        // 2. GPT-4.1 
        // 3. First available model
        
        let fallbackModel = null;
        
        // Priority 1: Claude 4 (Claude 3.5 Sonnet)
        fallbackModel = availableModels.find(m => 
            m.family.toLowerCase().includes('claude') && 
            (m.family.includes('3.5') || m.family.includes('4'))
        );
...
async function reviewWithInMemoryTemplate(diffContent, stream, changeType = 'Changes', selectedModel = null, context = null, request = null) {
    ...
    try {
        ...
        if (fs.existsSync(instructionPath)) {
            ...
            stream.markdown(`✅ **Template loaded:** \`${templateSource === 'workspace' ? 'instructions' : 'extension/templates'}/review-changes.md\`\n`);
        } else {
        ...
        stream.markdown('🔄 **In-memory processing completed**\n');
        stream.markdown(`**Diff Length:** ${diffContent ? diffContent.length : 0} characters\n`);
        stream.markdown(`**Template applied:** ${reviewContent !== instructionTemplate ? 'Successfully' : 'Failed to replace'}\n\n`);
        stream.markdown('---\n\n');
...
async function performInMemoryReview(reviewContent, diffContent, stream, changeType, selectedModel = null, context = null, request = null) {
    ...
    try {
        ...
        stream.markdown(`📋 **Using template:** \`${templateSource === 'workspace' ? 'instructions' : 'extension/templates'}/review-changes.md\`\n\n`);
    } catch (error) {
    ...
    stream.markdown('# 📋 Code Review Template Applied\n\n');
    stream.markdown('**Template:** `instructions/review-changes.md`\n');
    stream.markdown(`**Changes:** ${changeType}\n\n`);
...
Focus on real issues, security concerns, and actionable recommendations for this Angular TypeScript project. Provide specific code examples for improvements.`)
        ...
        // Show which model we're starting with
        stream.markdown(`🎯 **Starting with model:** ${currentModel.family}\n\n`);
...
async function handleFilePathChangesReview(filePath, stream, selectedModel = null, context = null, request = null) {
    try {
        ...
        stream.markdown(`# 🔍 Changes Review: \`${path.basename(cleanPath)}\`\n\n`);
        stream.markdown(`📂 **Full Path:** \`${cleanPath}\`\n\n`);
...
async function handleFilePathReview(filePath, stream, requestedModel = null, context = null) {
    try {
        ...
        stream.markdown(`# � File Review: \`${path.basename(cleanPath)}\`\n\n`);
        stream.markdown(`📂 **Full Path:** \`${cleanPath}\`\n\n`);
...
async function reviewFileWithDiff(gitDiff, filePath, stream, changeType, context = null, request = null) {
    stream.markdown(`## 📋 ${changeType} Review\n\n`);
...
async function reviewCurrentFileContent(filePath, stream, requestedModel = null, context = null) {
    try {
        ...
        try {
            ...
            // Try workspace instructions first, then fallback to extension templates
            let templatePath = path.join(workspaceFolder.uri.fsPath, 'instructions', 'review-file.md');
            let templateSource = 'workspace';
            ...
            if (fs.existsSync(templatePath)) {
                ...
                stream.markdown(`✅ **Template loaded:** \`${templateSource === 'workspace' ? 'instructions' : 'extension/templates'}/review-file.md\`\n\n`);
...
Provide specific examples and actionable recommendations.`;
            ...
            if (model) {
                ...
                while (attemptCount < maxAttempts) {
                    try {
                        console.log(`📤 File review attempt ${attemptCount + 1}: Sending request to model:`, currentModel.family);
                        ...
                        // Clear loading and start streaming content
                        stream.markdown('---\n\n');
                        ...
                        for await (const fragment of response.text) {
                            if (isFirstFragment) {
                                // Show streaming has started
                                stream.markdown('📡 **Live streaming analysis results...**\n\n');
                                isFirstFragment = false;
                            }
                    ...
                    if (fragmentCount % 25 === 0) {
                        console.log(`📊 File review progress: ${fragmentCount} fragments, ~${wordCount} words`);
                    }
                }
                
                stream.markdown('\n\n🎉 **Streaming complete!** ');
                stream.markdown(`(${fragmentCount} fragments, ~${wordCount} words processed)`);
                stream.markdown('\n\n---\n\n');
...
async function getUnifiedModel(stream, requestedModel = null, chatContext = null, request = null) {
    ...
    // Debug output in markdown for easier reading
    stream.markdown('### 🔍 DEBUG: Model Selection\n');
    stream.markdown(`**Request.model:** ${request?.model ? `${request.model.family} (${request.model.vendor})` : 'null'}\n\n`);
    stream.markdown(`**Requested Model Param:** ${requestedModel ? JSON.stringify(requestedModel) : 'null'}\n\n`);
    stream.markdown(`**Chat Context:** ${chatContext ? 'Available' : 'null'}\n\n`);
    ...
    if (request && request.model) {
        ...
        stream.markdown(`🎯 **Found in request.model:** \`${request.model.family}\` (${request.model.vendor})\n\n`);
        stream.markdown(`🤖 **Using selected model:** \`${request.model.family}\` (${request.model.vendor})\n\n`);
        return request.model;
    } else {
        console.log('❌ No model found in request.model');
        stream.markdown('❌ **No model found in request.model**\n\n');
    }
    
    // PRIORITY 2: Use passed requestedModel object
    if (requestedModel && typeof requestedModel === 'object' && requestedModel.family) {
        stream.markdown(`🎯 **Found in requestedModel param:** \`${requestedModel.family}\` (${requestedModel.vendor})\n\n`);
        stream.markdown(`🤖 **Using requested model:** \`${requestedModel.family}\` (${requestedModel.vendor})\n\n`);
        console.log('Using requested model object:', requestedModel);
        return requestedModel;
    } else {
        stream.markdown('❌ **No requestedModel param provided**\n\n');
    }
    
    // PRIORITY 3: Try to get the current model from chat context  
    if (chatContext) {
        console.log('🔍 Checking for model in chatContext...');
        stream.markdown('🔍 **Checking chatContext for model...**\n\n');
        ...
        if (chatContext.model) {
            ...
            stream.markdown(`✅ **Found at chatContext.model:** ${JSON.stringify(chatContext.model)}\n\n`);
            currentModel = chatContext.model;
        } else {
            console.log('❌ No model found at chatContext.model');
            stream.markdown('❌ **No model at chatContext.model**\n\n');
        }
        ...
        if (currentModel) {
            console.log('🎯 Using model from context:', currentModel);
            stream.markdown(`🤖 **Using selected chatbox model:** \`${currentModel.family}\` (${currentModel.vendor})\n\n`);
            console.log('Using selected chatbox model:', currentModel);
            return currentModel;
        } else {
            console.log('⚠️ No model found in any chatContext location');
            stream.markdown('❌ **No model found in chatContext**\n\n');
        }
    } else {
        stream.markdown('❌ **No chatContext provided**\n\n');
    }
    ...
    stream.markdown('### ⚠️ FALLBACK: Using available models detection\n');
    stream.markdown('**Reason:** No model found in request.model, requestedModel, or chatContext\n\n');
    ...
    try {
        ...
        if (models.length > 0) {
            ...
            const modelList = models.map(m => `${m.family} (${m.vendor})`).join(', ');
            stream.markdown(`🤖 **Available models:** ${modelList}\n\n`);
            ...
            if (claude4Model) {
                stream.markdown(`🎯 **FALLBACK 1 - Found Claude 4:** \`${claude4Model.family}\` (${claude4Model.vendor})\n\n`);
                ...
                if (gpt41Model) {
                    stream.markdown(`🎯 **FALLBACK 2 - Found GPT-4.1:** \`${gpt41Model.family}\` (${gpt41Model.vendor})\n\n`);
                    ...
                    stream.markdown(`🎯 **FALLBACK 3 - Using first available:** \`${models[0].family}\` (${models[0].vendor})\n\n`);
        ...
        stream.markdown(`⚠️ **Could not get models:** ${error.message}\n\n`);
